...
<div style="margin-left: 150px; margin-right: 150px;" class="bakgrund">
  ...
  $conn = new mysqli($servername, $username, $password, $dbname);
   ...
   if($conn->connect_error)
    ...
    if($omrade=='')
    {
        echo '<h2 style="float; text-align:center; padding-right:30px; ">Ditt postnummer verkar inte existera. <br> Vänligen kontakta admin! </h2>';//$week visar enbart veckan
    }
    else{
        $postNummer=$row['Postnummer'];
        setcookie('PostNr', $postNummer, time() +3600);
        echo '<h2 style="float; text-align:center;">Välj lokal hos området <br>'.$omrade.'</h2>';//$week visar enbart veckan
        echo '<h2 style="float; text-align:center;">Vecka: <br>'.$week.'</h2>';//$week visar enbart veckan
